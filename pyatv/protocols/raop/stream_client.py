...
class ControlClient(asyncio.Protocol):
    ...
    def _find_member(self, addr) -> Optional[StreamMember]:
        ...
        # buddy: the gap is never repaired and that speaker drops a packet for
...
class StreamClient:
    ...
    async def initialize(self, properties: Mapping[str, str]):
        ...
        # Set up receivers one after the other (not concurrently) as receivers
        # have been observed to fail when set up in parallel
        ...
        try:
            ...
            # earlier one has fully set up - the buddy is busy, or rebooting, or
    ...
    async def _teardown_member(self, protocol: StreamProtocol) -> None:
        ...
        try:
            ...
            # transport meant that anything failing in between - a busy buddy, an
