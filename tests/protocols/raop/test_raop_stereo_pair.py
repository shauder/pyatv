...
from tests.protocols.raop.test_raop import PARTNER_CREDENTIALS
...
@pytest.mark.parametrize("raop_properties", [{"et": "0"}])
async def test_partner_setup_failure_tears_down_the_primary(
    raop_pair_client, raop_usecase2, raop_state
):
    # The partner demands a password the session does not have (credentials and
    # password live on the shared context, so only the primary's are known).
    # Whatever the cause - busy, rebooting, rejected pairing - a partner that fails
...
async def pair_session_fixture(raop_device, raop_device2, raop_properties):
    ...
    async def _setup(credentials=None, partner_credentials=None):
        partner_port = raop_device2.get_port(Protocol.RAOP)
        storage = MemoryStorage()

        if partner_credentials:
            # Storage as pairing the other half leaves it: its own settings,
            # under its own identifier
            partner_config = AppleTV("127.0.0.1", "Other half")
            partner_config.add_service(
                ManualService("partner_id", Protocol.RAOP, partner_port, {})
            )
            partner_settings = await storage.get_settings(partner_config)
            partner_settings.protocols.raop.credentials = partner_credentials
        ...
        service.stereo_pair_address = f"127.0.0.1:{partner_port}"
...
async def test_paired_pair_drives_both_halves_with_own_credentials(
    ...
    client = await pair_session(
        credentials=CREDENTIALS, partner_credentials=PARTNER_CREDENTIALS
    )
    ...
    assert protocols[1].credentials == parse_credentials(PARTNER_CREDENTIALS)
...
async def test_paired_other_half_is_driven_by_an_unpaired_device(
    ...
    client = await pair_session(partner_credentials=PARTNER_CREDENTIALS)
