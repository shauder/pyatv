...
server, which both halves take their time from.

That a shared timeline is what keeps two speakers together — rather than the timing
protocol being PTP rather than NTP — was established on a bonded pair of HomePods
running audioOS 27, by ear, over runs of 30 seconds and five minutes. That work was
done with a separate sender, so it is evidence for the approach and not a measurement
of this implementation, and a listener's verdict over those durations is not a
measurement of drift either.
...
* The AirPlay record is the only place `tsid` is published, and it does not always
  arrive inside a scan window. A half that answered without it looks exactly like a
  speaker that is not in a pair, and comes back under its RAOP identifier rather
  than its AirPlay one, so pyatv logs a warning naming the speaker instead of
  quietly streaming to one half. A multicast scan asks the host directly for the type
  it stayed silent on, which removed this on every network it has been measured on.
  Scanning again or scanning for longer does not: a speaker that has gone quiet on a
  type stays quiet for as long as a scan is willing to wait.
...
* Streaming twice in rapid succession can fail: for a few seconds after a session is
  torn down a pair goes through a transitional state, with both halves advertising
  themselves as leaders, and it may refuse to set up a new session while it is there.
  The state was observed to last four to ten seconds; no threshold beyond that has
  been measured, so retry rather than trusting a fixed wait.
* More than two receivers is not supported: only one partner per device, and nothing
...
`protocols.raop.stereo_pair_address` overrides what scanning worked out. Set it to
...
settings.protocols.raop.stereo_pair_address = "10.0.0.20"
...
atvremote -s 10.0.0.10 --id <identifier> \
    change_setting=protocols.raop.stereo_pair_address,10.0.0.20 stream_file=sample.mp3
```
...
Which half renders which channel is a setting in the Home app, owned by the pair. It
is not published anywhere on the network — not in a TXT record, not in a session
response — and it can be changed by whoever owns the speakers at any time.

On the pair used here the names were at one point assigned to the opposite sides, so
the half named "Left" rendered the right channel until the owner swapped them back.
That is the whole point: a name is whatever someone typed, and it can disagree with
the channel today and agree with it tomorrow. Do not infer the role from a name, in
your own code or when setting `stereo_pair_address`. Which half is which does not
affect grouping, and the only way to find out is to listen.
