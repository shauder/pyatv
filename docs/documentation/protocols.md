...
A receiver can be told that its session belongs to a *playback group*, i.e. that
several receivers play the same audio as one room (a stereo pair being the most
common case). That is done with a few extra keys in the body of the AirPlay v2
session `SETUP`:

| Key | Value | Description |
| --- | ----- | ----------- |
| groupUUID | 6B2C2F4E-... | Identifies the group, same value for every member
| senderSupportsRelay | true | Receiver ignores `groupUUID` unless this is true
| groupContainsGroupLeader | false | True if the leader of the group is a receiver

`groupUUID` is generated by the sender, once per group, and is a **version 4**
(i.e. random) UUID in upper case. A device derives the identifiers of the groups
it forms by itself (e.g. a stereo pair) as name based, *version 5* UUIDs, so the
version tells whether a group was set up by a sender or by the devices
themselves. A sender generating a version 5 UUID would blur that distinction.

`groupContainsGroupLeader` is false as a sender is not a receiver that others can
be told to follow. Note that a receiver leading a group does *not* relay audio to
the other members on behalf of a third party sender: every member is streamed to
directly, with its own session.

**A playback group is not a stereo pair, but it does not replace one either.** The
group a sender sets up this way is a set of receivers playing the same audio in
sync. That is not the same thing as the stereo pair two speakers form between
themselves — and two speakers that are bonded as a pair keep that bond while they
are members of a sender's group: they adopt the sender's group and still render
their own left and right channels out of the stereo stream each of them is sent.
The sender does nothing channel-related.

Group membership is announced in cleartext in the mDNS TXT record (`gid`, `igl`
and `gcgl`, listed under service discovery above), so this can be read off the
network. Observed on a bonded pair of HomePods running audioOS 27, with samples
taken through live sessions:

| Session | What both halves advertise |
| ------- | -------------------------- |
| No session (bonded) | one shared `gid`; one half `igl=1 gcgl=1`, the other `igl=0 gcgl=1`
| Grouped, NTP timeline | `igl=0`, `gcgl=0` and the *sender's* `gid`, on both halves
| Grouped, shared PTP timeline | `igl=0`, `gcgl=0` and the *sender's* `gid`, on both halves
| No `groupUUID` sent | `igl=1` on both halves, each with a `gid` of its own

It is the `groupUUID` that decides this, not the timing protocol: the two grouped
rows were taken on one pair with the clock as the only difference between them, and
the bottom row is what a session that names no group produces on the same pair.
`timingProtocol: NTP` gives the members a common clock and a common anchor, and that
is enough both for the group to form with the signature above and for a bonded pair
to go on rendering its own channels (that second part is a listener's verdict on one
pair, not a measurement). Receivers sharing a PTP timeline can in
addition tight-sync to each other; nothing above needs it. pyatv implements NTP
timing only.

A session that sends no `groupUUID` is what dissolves a pair into two groups of
one, each half playing the full mix — the timing protocol is not what decides it.
The account of the listening test is on the
[stereo pairs](/development/stream/) page.

Nothing observed advertises which half of a pair renders which channel — not the
TXT record, not any session response. The names are assigned by whoever set the
speakers up and need not match the roles, so a sender must not infer a channel role
from a receiver's name.

A side effect worth knowing is that the TXT record makes grouping testable without
listening to anything: after a session that has grouped its receivers correctly,
every member advertises `igl=0`, `gcgl=0` and the sender's `gid`.
